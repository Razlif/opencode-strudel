...
Mini-notation rules:
- `~` = rest
- `[]` = subdivision inside the beat
- `<>` = alternation across cycles or larger spans
- `!` = repeat
- `@` = duration
- use `s(...)` for samples and drums
- use `note(...)` or `n(...)` for pitched material
- use one readable phrase before writing one dense expression
...
Sound-role guidance:
- bass: `gm_synth_bass_1`, `gm_fretless_bass`, `gm_electric_bass_finger`, `gm_electric_bass_pick`
- harmony bed: `gm_pad_warm`, `gm_pad_choir`, `gm_string_ensemble_1`, `piano`, `gm_drawbar_organ`
- lead and color: `gm_oboe`, `gm_clarinet`, `gm_tenor_sax`, `gm_lead_2_sawtooth`, `gm_violin`
- club drums: `RolandTR909`, `RolandTR808`, `LinnDrum`
- rock and dry drums: `SequentialCircuitsDrumtracks`, `BossDR110`

Density guidance:
- low layers should carry weight
- harmony beds should be audible but not dominate the groove
- hats, shakers, and color layers usually stay quieter
- not every layer should be equally dense or equally loud
...
- `A A' B A`
- `intro -> groove -> lift -> peak -> outro`
- `minimal -> add motion -> add color -> strip back`
- `home -> contrast -> return`
...
Variation rule:
- never repeat the exact same phrase more than twice in a row
- by the third pass, make some change
...
Teach melody as phrase-shape, not just note order.

Rules:
- melody should feel like talking
- build call and response inside the same line
- write in phrase blocks:
  - opening
  - answer
  - development
  - landing
- use inner turns inside longer tones
- keep the line moving through contour and register
- mix held notes with shorter internal motion
- never repeat the exact same phrase more than twice in a row
- by the third pass, change the ending, pickup, ornament, or landing
- keep one recognizable fingerprint in the line even when it varies
...
```js
let prog_funk = "<Dm7 G7>/4"
let groove_funk = "bd ~ sd ~ bd ~ sd ~"
let motif_funk = "<0 ~ [2 1] ~>/4"

let drums_funk = stack(
  s(groove_funk).bank("RolandTR909").gain(0.42),
  s("[~ hh] ~ [~ hh] ~").bank("RolandTR909").gain(0.12),
)

let bass_funk = n("0 ~ 0 [2 0]")
  .chord(prog_funk)
  .anchor("d2")
  .mode("root")
  .voicing()
...
let bed_funk = n("[0,2,4]@0.5 ~ [0,2,4]@0.5 ~")
  .chord(prog_funk)
  .anchor("f4")
  .voicing()
  .s("gm_epiano1")
...
let guitar_bed = sel.pickRestart([
  note("<[d4,f4,a4] [g3,b3,d4]>/2"),
  note("<[c4,e4,g4] [f3,a3,c4]>/2"),
]).s("gm_overdriven_guitar").gain(0.22)

let bass = sel.pickRestart([
  note("<d2 a1 g1 d1>/4"),
  note("<c2 g1 f1 c1>/4"),
]).s("gm_electric_bass_pick").lpf(1600).gain(0.3)

let lead = sel.pickRestart([
  n("<[0@3 1@2 [2 1]@1 2@2 3@4] [4@2 3@1 [2 3]@1 2@2 1@3 0@3] [2@2 3@1 [4 3]@1 2@2 1@2 0@4] [1@2 [0 1]@1 -1@1 0@2 2@2 0@4]>").scale("d4:minor"),
  n("<[0@2 2@2 [3 2]@1 3@1 4@3 2@3] [3@2 4@1 [5 4]@1 3@2 2@2 1@4] [4@2 3@1 [2 1]@1 2@2 0@2 1@4] [2@2 [1 0]@1 -1@1 0@2 3@2 1@4]>").scale("c4:minor"),
...
### 15. Activation-Window Arranging
...
```js
let harm = note("<[c4,eb4,g4] [ab3,c4,eb4] [f3,ab3,c4] [g3,b3,d4]>/4")

let bed = harm
  .s("gm_pad_warm")
  .gain(0.18)

let bass = "<0@8 1@8 ~@8 0@8>/4"
  .pickRestart([
    note("c2 ~ c2 ~ eb2 ~ c2 ~"),
    note("c2 ~ g1 ~ ab1 ~ g1 ~"),
  ])
...
// @song_tracks
let track_section_a_kick = s("bd*4")
  .bank("RolandTR909")
  .gain(0.42)

let track_section_a_hats = s("[~ hh]*4")
  .bank("RolandTR909")
  .gain(0.08)

let track_section_a_bass = note("<c2 ~ c2 ~ ab1 ~ ab1 ~>/4")
  .s("gm_synth_bass_1")
  .lpf(1100)
  .gain(0.28)

let track_section_a_bed = note("<[c4,eb4,g4] [ab3,c4,eb4]>/4")
  .slow(2)
  .s("gm_pad_warm")
  .attack(0.2)
  .release(0.45)
...
let track_section_a_high = note("<c5 ~ [eb5 c5] ~ ab4 ~ [c5 ab4] ~>/4")
  .s("gm_lead_2_sawtooth")
  .gain(0.14)

// @song_sections
let section_section_a = stack(
  track_section_a_kick,
  track_section_a_hats,
  track_section_a_bass,
  track_section_a_bed,
  track_section_a_high,
)
