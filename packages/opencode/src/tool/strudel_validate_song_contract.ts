...
import DESCRIPTION from "./strudel_validate_song_contract.txt"
...
export const StrudelValidateSongContractTool = Tool.define("strudel_validate_song_contract", {
  description: DESCRIPTION,
  parameters: z.object({
    code: z.string().describe("The full canonical Strudel song file to validate"),
    goal: z.string().optional().describe("Optional short note about what the file is meant to represent"),
  }),
  async execute(args) {
    const result = check(args.code)
    const output = {
      ok: result.ok,
      summary: result.ok ? "Canonical Strudel song contract passed" : "Canonical Strudel song contract failed",
      errors: result.err.map((message) => ({ kind: "contract", message })),
      warnings: [],
      notes: [],
      ...(args.goal ? { goal: args.goal } : {}),
    }
    return {
      title: result.ok ? "Strudel song contract ok" : "Strudel song contract failed",
